...
function saveUserHistory(username, historyArr) {
  try {
    // Chỉ lưu tối đa 100 bài gần nhất để tránh localStorage đầy
    const toSave = historyArr.slice(0, 100);
    localStorage.setItem(getHistoryKey(username), JSON.stringify(toSave));
  } catch (e) {
    console.warn("Không lưu được lịch sử (có thể localStorage đầy):", e);
...
function checkLoginState() {
  ...
  if (saved) {
    try {
      ...
      if (currentUser && currentUser.username && ACCOUNTS[currentUser.username]) {
        // Kiểm tra registry vĩnh viễn khi khôi phục phiên
        const deviceCheck = canLoginOnThisDevice(currentUser.username);
        if (!deviceCheck.ok) {
          // Thiết bị này chưa được đăng ký và tài khoản đã đủ 2 slot → buộc đăng xuất
          currentUser = null;
        ...
        // Cập nhật lastLogin trong registry
        registerCurrentDevice(currentUser.username);
        // restore avatar / name overrides
        try {
  ...
  // Chưa đăng nhập → vẫn cho xem giao diện
  currentUser = null;
  examHistory = [];
  updateUIAfterLogin();
  return false;
...
function updatePresence(status) {
  ...
  try {
    // Key theo username + deviceId để hỗ trợ nhiều thiết bị
...
function getPresenceList() {
  const result = [];
  const now = Date.now();
  const ONLINE_MS = 120000; // 2 phút
  const byUser = {}; // username -> best presence (exam ưu tiên hơn online)
  try {
